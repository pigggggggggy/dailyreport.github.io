...
5. 打开 Supabase Authentication -> Users，手动新增两个用户：
   - `zjw@dailyreport.local`，密码设为 `zjw123`
   - `sxk@dailyreport.local`，密码设为 `sxk123`
6. 把修改后的文件提交到 GitHub Pages 仓库。

配置完成后，页面会先要求登录。协作者打开同一个网址后，用自己的账号密码登录，就可以添加学习日志、LeetCode 记录和项目进展。
...
当前 SQL 已收紧为 Supabase Auth 登录用户可读写，匿名访问者不能直接读取、添加或删除记录。`config.js` 里只保存账号到邮箱的映射，不保存密码；密码请只在 Supabase Authentication 里设置。
